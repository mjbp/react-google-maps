...
_ref = [OverlappingMarkerSpiderfier, p];
...
function OverlappingMarkerSpiderfier(maps, map, opts) {
...
this.projHelper = new this.maps.OverlayView();
this.projHelper.setMap(this.map);
this.projHelper.draw = function() {};
...
for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
  e = _ref1[_j];
  this.maps.event.addListener(this.map, e, (function(_this) {
    return function() {
...
marker['_oms'] = true;
listenerRefs = [
  this.maps.event.addListener(marker, this['event'], (function(_this) {
...
if (!this['markersWontHide']) {
  listenerRefs.push(this.maps.event.addListener(marker, 'visible_changed', (function(_this) {
    return function() {
...
if (!this['markersWontMove']) {
  listenerRefs.push(this.maps.event.addListener(marker, 'position_changed', (function(_this) {
    return function() {
...
for (_j = 0, _len1 = listenerRefs.length; _j < _len1; _j++) {
  ...
  this.maps.event.removeListener(listenerRef);
}
delete marker['_oms'];
...
for (i = _j = 0; 0 <= count ? _j < count : _j > count; i = 0 <= count ? ++_j : --_j) {
  ...
  _results.push(new this.maps.Point(centerPt.x + legLength * Math.cos(angle), centerPt.y + legLength * Math.sin(angle)));
}
return _results;
};
...
for (i = _j = 0; 0 <= count ? _j < count : _j > count; i = 0 <= count ? ++_j : --_j) {
  ...
  pt = new this.maps.Point(centerPt.x + legLength * Math.cos(angle), centerPt.y + legLength * Math.sin(angle));
  ...
  for (_j = 0, _len1 = footPts.length; _j < _len1; _j++) {
    ...
    if (this['legColors']['highlighted'][this.map.mapTypeId] !== this['legColors']['usual'][this.map.mapTypeId]) {
      ...
      marker['_omsData'].hightlightListeners = {
        highlight: this.maps.event.addListener(marker, 'mouseover', highlightListenerFuncs.highlight),
        unhighlight: this.maps.event.addListener(marker, 'mouseout', highlightListenerFuncs.unhighlight)
      };
    }
...
for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
  ...
  if (marker['_omsData'] != null) {
    ...
    if (listeners != null) {
      this.maps.event.removeListener(listeners.highlight);
      this.maps.event.removeListener(listeners.unhighlight);
    }
...
numPts = pts.length;
return new this.maps.Point(sumX / numPts, sumY / numPts);
};
